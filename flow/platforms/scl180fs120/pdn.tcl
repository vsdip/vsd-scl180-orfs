...
define_pdn_grid -name {grid} -voltage_domains {CORE} -pins {M5}
...
# Explicit M1 rails instead of -followpins (avoids PDN-0179 channel repair)
# Row height = 5.600um (CoreSite sh=5600 DBU)
# VSS rail thickness in LEF ~0.740um at y=0..0.740
# VDD rail thickness in LEF ~0.740um at y=4.860..5.600
add_pdn_stripe -grid {grid} -layer {M1} -width {0.740} -pitch {5.600} -offset {0.000}
add_pdn_stripe -grid {grid} -layer {M1} -width {0.740} -pitch {5.600} -offset {4.860}


# 1.5) Add sparse bridge straps so connects have shapes to hit
# M2 typically vertical, M3 typically horizontal (adjust pitch later)
#add_pdn_stripe -grid {grid} -layer {M2} -width {1.200} -pitch {60.000} -offset {30.000}
add_pdn_stripe -grid {grid} -layer {M3} -width {1.200} -pitch {60.000} -offset {30.000}
