...
# Repair tie fanout (optional; some platforms have no tiehi/tielo)
if { !$::env(SKIP_REPAIR_TIE_FANOUT) } {

  set have_tielo [expr {[info exists ::env(TIELO_CELL_AND_PORT)] && $::env(TIELO_CELL_AND_PORT) ne ""}]
  set have_tiehi [expr {[info exists ::env(TIEHI_CELL_AND_PORT)] && $::env(TIEHI_CELL_AND_PORT) ne ""}]

  if { !$have_tielo && !$have_tiehi } {
    puts "INFO: No TIEHI/TIELO cells defined; skipping repair_tie_fanout."
  } else {
    # This needs to come before any call to remove_buffers. You could have one
    # tie driving multiple buffers that drive multiple outputs.
    ...
    if { $have_tielo } {
      ...
      puts "INFO: TIELO not defined; skipping tie-lo fanout repair."
    }
    ...
    if { $have_tiehi } {
      ...
      puts "INFO: TIEHI not defined; skipping tie-hi fanout repair."
    }
  }
}
